...
const CarbonCalculator = {
    ...
    calculate(inputs) {
        ...
        // 1. TRANSPORTATION
        // Car: miles * factor. Convert kg to tonnes (/ 1000)
        const carMiles = parseFloat(inputs.carMiles) || 0;
        const carType = inputs.carType || 'petrol';
        const carFactor = this.FACTORS.car[carType] || this.FACTORS.car.petrol;
        transportCar = (carMiles * carFactor) / 1000;

        // Public Transit: weekly hours * speed * 52 weeks * factor. Convert kg to tonnes
        const transitHours = parseFloat(inputs.transitHours) || 0;
        ...
        // Flights: quantity * factors. Convert kg to tonnes
        const flightsShort = parseFloat(inputs.flightsShort) || 0;
        const flightsLong = parseFloat(inputs.flightsLong) || 0;
        ...
        // 2. HOME ENERGY
        const cleanEnergyShare = (parseFloat(inputs.cleanEnergyShare) || 0) / 100;
        
        // Electricity: monthly bill -> annual kWh -> emissions * (1 - clean share). Convert kg to tonnes
        ...
        // Heating: monthly bill -> annual energy -> emissions. Convert kg to tonnes
        ...
        if (heatingSource === 'gas') {
            ...
            // Electric heating matches electricity source intensity and respects clean energy share
        ...
        // 3. DIET & FOOD
        // Dietary profile baseline
        const dietType = inputs.dietProfile || 'medium-meat';
        const dietBaseline = this.FACTORS.diet[dietType] || this.FACTORS.diet['medium-meat'];
        ...
        // 4. WASTE & CONSUMPTION
        // Shopping level baseline
        const shoppingLevel = parseInt(inputs.shoppingHabits) || 3;
        const shoppingBaseline = this.FACTORS.shopping[shoppingLevel] || this.FACTORS.shopping[3];

        // Waste baseline minus recycling checklist items
        ...
        return {
            categories: {
                ...
                food: {
                    total: Number(foodEmissions.toFixed(2))
                },
                waste: {
                    total: Number(wasteEmissions.toFixed(2))
                }
            },
